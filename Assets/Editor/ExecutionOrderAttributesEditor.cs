...
namespace ExecutionOrder.Editor
{
		...
		static List<ScriptExecutionOrderDependency> GetExecutionOrderDependencies(Dictionary<Type, MonoScript> types)
		{
			...
			foreach (var kvp in types)
			{
				...
				if (hasExecuteAfterAttribute)
				{
					...
					{
						Debug.LogError(
							string.Format("Script {0} has both [ExecutionOrder] and [ExecuteAfter] attributes. Ignoring the [ExecuteAfter] attribute.",
							              script.name), script);
						continue;
					}
					...
					foreach (var attribute in attributes)
					{
						if (attribute.orderIncrease < 0)
						{
							Debug.LogError(
								string.Format(
									"Script {0} has an [ExecuteAfter] attribute with a negative orderIncrease. Use the [ExecuteBefore] attribute instead. Ignoring this [ExecuteAfter] attribute.",
									script.name), script);
							continue;
						}
						...
						if (!attribute.targetType.IsSubclassOf(typeof(MonoBehaviour)) && !attribute.targetType.IsSubclassOf(typeof(ScriptableObject)))
						{
							Debug.LogError(
								string.Format(
									"Script {0} has an [ExecuteAfter] attribute with targetScript={1} which is not a MonoBehaviour nor a ScriptableObject. Ignoring this [ExecuteAfter] attribute.",
									script.name, attribute.targetType.Name), script);
							continue;
						}
				...
				if (hasExecuteBeforeAttribute)
				{
					...
					{
						Debug.LogError(
							string.Format("Script {0} has both [ExecutionOrder] and [ExecuteBefore] attributes. Ignoring the [ExecuteBefore] attribute.",
							              script.name), script);
						continue;
					}
					...
					{
						Debug.LogError(
							string.Format("Script {0} has both [ExecuteAfter] and [ExecuteBefore] attributes. Ignoring the [ExecuteBefore] attribute.",
							              script.name), script);
						continue;
					}
					...
					foreach (var attribute in attributes)
					{
						if (attribute.orderDecrease < 0)
						{
							Debug.LogError(
								string.Format(
									"Script {0} has an [ExecuteBefore] attribute with a negative orderDecrease. Use the [ExecuteAfter] attribute instead. Ignoring this [ExecuteBefore] attribute.",
									script.name), script);
							continue;
						}
						...
						if (!attribute.targetType.IsSubclassOf(typeof(MonoBehaviour)) && !attribute.targetType.IsSubclassOf(typeof(ScriptableObject)))
						{
							Debug.LogError(
								string.Format(
									"Script {0} has an [ExecuteBefore] attribute with targetScript={1} which is not a MonoBehaviour nor a ScriptableObject. Ignoring this [ExecuteBefore] attribute.",
									script.name, attribute.targetType.Name), script);
							continue;
						}
